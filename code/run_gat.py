...
# data
#################################
dataset = loadAFL(CONFIG['dataset'], split=CONFIG['split'], need_dgl=True)
...
optimizer = torch.optim.Adam([{
    'params': net.gc1.parameters(),
    'weight_decay': CONFIG['decay']
}, {
    'params': net.gc2.parameters(),
    'weight_decay': CONFIG['decay']
}],
                             lr=CONFIG['lr'])
...
for epoch in range(1, CONFIG['epoch'] + 1):
    ...
    with timer(name='total'):
        ...
        train_loss = F.nll_loss(train_logp[dataset['train mask']],
                                labels[dataset['train mask']])
        train_pred = train_logp.argmax(dim=1)
        train_acc = torch.eq(
            train_pred[dataset['train mask']],
            labels[dataset['train mask']]).float().mean().item()
        ...
        with torch.no_grad():
            ...
            val_loss = F.nll_loss(val_logp[dataset['valid mask']],
                                  labels[dataset['valid mask']]).item()
            val_pred = val_logp.argmax(dim=1)
            val_acc = torch.eq(
                val_pred[dataset['valid mask']],
                labels[dataset['valid mask']]).float().mean().item()
            tes_acc = torch.eq(
                val_pred[dataset['test mask']],
                labels[dataset['test mask']]).float().mean().item()
...
with torch.no_grad():
    ...
    test_loss = F.nll_loss(test_logp[dataset['test mask']],
                           labels[dataset['test mask']]).item()
    test_pred = test_logp.argmax(dim=1)
    test_acc = torch.eq(test_pred[dataset['test mask']],
                        labels[dataset['test mask']]).float().mean().item()
